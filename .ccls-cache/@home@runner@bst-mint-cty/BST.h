 ...
 new_node = (TreeNodePtr)malloc(sizeof(struct TreeNode));
   t=new TreeNode;
     ...
     if((*t)->data>=value) //left
      inserted_R(&t->leftPtr,value); //right
     else 
      inserted_R(&t->rightPtr,value); //right
...
void insertNode(BST *b, int value) {
  ...
  if (new_node) {
    ...
    /*First Node*/
    if (!b->root)
      b->root = new_node; // bไม่มีroot bเป็น newnode
    else {
      while (!inserted) {
        if (t->data >= value) { // 5(ตัวที่ t ชี้ไปตอนแรก data)>3(value) ไปซ้าย
...
void printfTree(TreeNodePtr treePtr , int x){
  ...
  printfTree(treePtr->rightPtr,x+1);
  ...
  printf("   ");
   ...
   printf("%5d\n", treePtr->data);
    ...
    printfTree(treePtr->leftPtr,x+1); 
  }                          
} // end
